# 🔄 Sonnet 4.6 への引き継ぎドキュメント

**作成日時**: 2026-02-13  
**プロジェクト**: 自由進度学習支援システム  
**現在の状態**: Phase 18-2 完了、データ生成基盤改善中
...
1. **Phase 1-18 完全実装済み**
   - 認証システム（Phase 4）
   - AI駆動型パーソナライゼーション（Phase 12）
   - レベル5理論体系統合（Phase 15-16）
   - リアルタイム適応学習（Phase 18-1）
   - 予測分析・A/Bテスト（Phase 18-2）

2. **テスト対策プランページ作成完了**
   - URL: `/static/test-preparation`
   - AI問題生成システム統合（iframe）
   - カリキュラムDB接続（3,807単元 → 現状150単元の実データのみ）

3. **学習カード生成モーダルに単元名DB統合**
   - `/api/curriculum/units` エンドポイント（実データのみ返す）
   - 学年・教科・教科書会社で絞り込み
   - ダミーデータは返さない設定
...
**問題点**:
- 5社×9学年×15教科 = 195組み合わせ中、わずか2組のみ実データ
- ユーザー要求: 「実データのみを使用、ダミーデータは使わない」
- システムは構築済みだが、データが致命的に不足

**試行したアプローチ**:
1. ❌ 単一Gemini API生成（タイムアウト、JSON抽出失敗多発）
2. ❌ ハイブリッド生成（既存データをFew-shot例として利用）
3. 🔄 並列5エンジン生成（タイムアウト、36/195成功で停止）
...
1. **実データ拡充を最優先**
   - 全学年・全教科のデータ生成を完了させる
   - 5社×9学年×15教科 = 195組み合わせすべて

2. **テスト対策と学習システムの連携**
   - テスト範囲入力: カリキュラムDB選択 + 自由テキスト入力の両方対応
   - 学習カード生成の理論・個性を活用
   - テスト対策で得た個別傾向を単元内自由進度学習にフィードバック

3. **学習カード枚数の動的調整**
   - 現在: 各コース6枚固定（計18枚）
   - 新方式: 単元の内容項目数に応じて動的に調整
   - 基準: 単元のボリューム、難易度、学習項目数
...
**主要テーブル**:
- `curriculum`: カリキュラムデータ（11,230件、実データ150件のみ）
- `cards`: 学習カード
