-- デモ用進捗データ投入スクリプト（クリーン版）
-- まず古いデータを削除してから新しいデータを投入

-- 古いデモデータを削除
DELETE FROM student_progress WHERE student_id IN (2, 3, 4) AND curriculum_id = 1;
DELETE FROM check_test_progress WHERE student_id IN (2, 3, 4) AND curriculum_id = 1;
DELETE FROM optional_problem_progress WHERE student_id IN (2, 3, 4) AND curriculum_id = 1;
...
-- 選択問題: まだ開始できない
-- ============================================================
INSERT INTO student_progress (student_id, curriculum_id, course_id, learning_card_id, status, understanding_level, help_requested_from, help_count, created_at, last_activity_at)
VALUES 
...
-- チェックテスト: 学習カード完了後に開始
INSERT INTO check_test_progress (student_id, curriculum_id, problem_number, status, attempts, completed_at, created_at)
VALUES
...
INSERT INTO optional_problem_progress (student_id, curriculum_id, optional_problem_id, status, started_at, completed_at, time_spent, created_at)
VALUES
