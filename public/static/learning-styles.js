...
class LearningStyleManager {
  ...
  addVisualEnhancements(text) {
    // 数字を強調
    text = text.replace(/(\d+)/g, '<span class="text-2xl font-bold text-blue-600">$1</span>')
    
    // 演算記号を強調
    text = text.replace(/([+\-×÷=])/g, '<span class="text-3xl font-bold text-red-500 mx-2">$1</span>')
