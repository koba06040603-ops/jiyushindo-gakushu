...
app.get('/api/curriculum/unit-suggestions', async (c) => {
  ...
  try {
    ...
    // Gemini APIで単元を生成
    const prompt = `あなたは日本の小学校・中学校の現役教師であり、教科書会社の教材作成にも携わった経験を持つ教育カリキュラムの専門家です。

【重要な使命】
このデータは実際の学校現場で先生方が使用されます。先生方の大切な授業準備に役立つよう、最高の品質と正確性をお願いします。

【タスク】
以下の条件に基づいて、実際の教科書に記載されている単元名を正確に30個リストアップしてください。
...
【厳守事項】
1. 必ず${textbook}が発行している${grade}の${subject}教科書に実際に記載されている単元名を使用すること
2. 単元名は教科書に記載されている正式名称をそのまま使用すること（省略や言い換えは不可）
3. 教科書の目次順（第1章から最終章まで）に正確に並べること
4. 学習指導要領に基づいた正しい内容であること
5. 各単元名は40文字以内で簡潔に記載すること

【品質基準】
- 実際の先生方が見て「これは正しい単元名だ」と確信できる内容にしてください
- 架空の単元名や推測による単元名は絶対に含めないでください
- もし確信が持てない単元がある場合は、その部分だけ一般的な単元名にしても構いません

【出力形式】
以下のJSON形式のみで返してください（説明文は一切不要です）：
{
  "units": [
    {"unit_name": "単元名1"},
    {"unit_name": "単元名2"},
