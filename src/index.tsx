...
5. 完全なJSON（{で始まり}で終わる）を出力すること`
    ...
    // 【最新】Gemini 2.0 Flash を使用
    const modelName = 'gemini-2.0-flash-exp'  // 最新の Gemini 2.0 Flash
...
`
    ...
    if (useHighQuality) {
      ...
      models = [
        { name: 'gemini-2.0-flash-exp', maxTokens: 16384 }  // Proのみ
      ]
      generationConfig = {
      ...
      models = [
        { name: 'gemini-2.0-flash-exp', maxTokens: 16384 },     // 最新・最も安定
        { name: 'gemini-2.0-flash-exp', maxTokens: 16384 },     // 高速
        { name: 'gemini-2.0-flash-exp', maxTokens: 16384 }        // 最高品質（フォールバック）
      ]
      generationConfig = {
        temperature: 0.7,
        maxOutputTokens: 16384,
...
【必須：JSONのみ出力】
{
  ...
  "retrieval_practice": {
    ...
    // フォールバック機能付きAPI呼び出し
    const models = ['gemini-2.0-flash-exp', 'gemini-2.0-flash-exp', 'gemini-2.0-flash-exp']
    let response
    let lastError
