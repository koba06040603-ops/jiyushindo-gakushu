...
// APIルート：段階的コース生成（1コース=2枚のカードを生成）
...
` : ''
    ...
    // 極限までシンプル化したプロンプト（2枚に削減）
    const prompt = `${grade}${subject}「${unitName}」学習カード2枚JSON:
{"course_name":"${courseInfo.name}","name":"${courseInfo.name}","label":"${courseInfo.label}","description":"${courseInfo.description}","color_code":"${courseInfo.color_code}","cards":[{"card_number":1,"card_title":"","card_type":"main","textbook_page":"","problem_description":"","new_terms":"","example_problem":"","example_solution":"","real_world_connection":"","answer":"","answer_explanation":"","hints":[{"hint_level":1,"hint_text":""},{"hint_level":2,"hint_text":""},{"hint_level":3,"hint_text":""}]},{"card_number":2,...}]}
2枚、各3ヒント必須。`
    ...
    // バリデーション
    if (!courseData.cards || courseData.cards.length < 2) {
      console.error('❌ バリデーションエラー:', {
        cards存在: !!courseData.cards,
        cards長さ: courseData.cards?.length || 0
      })
      throw new Error(`カードが2枚未満です: ${courseData.cards?.length || 0}枚`)
    }
