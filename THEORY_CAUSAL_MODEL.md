...
## Part 0: 設計思想 — なぜ因果モデルが必要か

現行システムの致命的問題は「理論の並列適用」にある。8つの理論をそれぞれ独立にスコア化し、ラベルを貼り、個別にアドバイスを出す。しかし学習は**一つの統合的プロセス**であり、理論は**同時に、相互に作用する**。

例：ある子が分数の問題で躓いた場合、
- ZPD（F7）が「難易度を下げる」と判断し、
- ATI（F4）が「構造化度を上げる」と判断し、
- SDT（F8）が「有能感を支える」と判断し、
- SRL（F5）が「方略を振り返らせる」と判断する

これらは**別々に起きるのではなく、一つの学習瞬間に同時に統合される**必要がある。

本設計書は、**Carroll (1963) の学校学習モデルを上位フレームワーク**とし、12理論がこのモデルのどのパラメータにどう作用するかを因果的に定義する。
...
- **核心**: SRLは「能力」ではなく「プロセス」。各位相で異なる介入が必要。AI は Think-Aloud でモデリングを提供する。
...
interface F12_Controls {
  ...
  boredom_intervention: boolean    // 退屈時の介入
  anxiety_intervention: boolean    // 不安時の介入
}
```
...
【介入ポイント】
- F7: 即座にZPD下限に戻す
- F8: Frustration Control 発動
- F5: 帰属再訓練（「方略が合わなかった」への誘導）
- F12: 感情調整（calming message）
```
...
| **長期（月〜学期単位）** | F2, F4, F5, F10 | 知能プロファイル更新、ATI重み調整、SRL発達段階判定 |
...
#### A. 自律的探究者（Autonomous Explorer）
**特徴**: 高い自己調整能力、安定した感情、豊富な方略、強い内発的動機  
**Carroll解釈**: 必要時間も有効時間も自力で最適化できる  
**制御方針**: 最小限の介入、自由度最大
...
#### D. 慎重な完璧主義者（Cautious Perfectionist）
**特徴**: 高い自己調整能力だが不安が強い、方略は豊富だが動機が外発的  
**Carroll解釈**: 方略はあるが不安が有効時間を食う。失敗への恐怖が挑戦を阻害。  
**制御方針**: 構造はやや高め、感情的安全を最優先、方略帰属の促進
...
#### G. 受動的依存者（Passive Dependent）
**特徴**: 低い自己調整能力、中程度の感情、方略が乏しい、動機が低い  
**Carroll解釈**: 必要時間が長く、有効時間が短い。両方を同時に改善する必要がある。  
**制御方針**: 高構造化 + SRL段階的育成 + 小さな成功体験の連鎖
...
#### H. 学習回避者（Learning Avoider）
**特徴**: 全軸低い。学習性無力感の兆候。最も注意が必要。  
**Carroll解釈**: 必要時間が極めて長く、有効時間が極めて短い。  
**制御方針**: 完全構造化 + 感情的安全の確保 + 教師の直接介入フラグ
...
async function computeIntegratedControls(
  ...
  if (recentBehavior.consecutive_errors >= 2) {
    ...
    baseControls.structure.difficulty_zpd_position *= 0.7  // 難易度を下げる
  ...
  if (recentBehavior.consecutive_successes >= 3) {
    ...
    baseControls.scaffold.fade_rate *= 1.3  // 足場を速く除去
  }
...
| 段階 | 実装内容 | 理論的焦点 |
|---|---|---|
| **Phase 1** | F4(ATI構造化), F7(足場6機能), F12(感情検出) | 瞬時の制御ループ |
| **Phase 2** | F5(SRL 3位相+4段階), F8(SDT 3欲求) | セッション単位の制御 |
| **Phase 3** | F6(認知方略+条件チェック), F1(多重符号化) | 短〜中期の最適化 |
| **Phase 4** | F2(多元入口), F3(Kolbサイクル完走) | 概念理解の深化 |
| **Phase 5** | F9, F10, F11 + アーキタイプ遷移モデル | 長期的発達 |

### 7.2 核心の実装原則

1. **すべての制御パラメータは連続値**: 離散的ラベル（高/中/低）は表示用のみ
2. **理論間の因果チェーンをコードに反映**: 一つの理論の出力が別の理論の入力になる
3. **条件付き適用**: すべての方略にはcanApply()チェックを実装
4. **随伴性（contingency）**: 学習者の反応に応じてリアルタイムに制御値を変化させる
5. **フェイディング**: 支援は常に「減らす方向」がデフォルト。増やすのは例外条件のみ
6. **社会的比較の完全排除**: 進度・成績の他者比較はシステム全体で禁止
7. **教師への連携**: Type H (学習回避者) は必ず人間の介入を促す

---

## Part 8: まとめ — このモデルが生み出す「個別」とは何か

従来の「個別最適化」は「正答率を見て難易度を上下する」だけだった。

本モデルが生み出す「個別」は以下の7層の同時最適化である：

1. **認知的入口の個別化**: どの感覚チャネル・知能入口から概念に接近するか（F1×F2）
2. **構造化度の個別化**: どの程度の自由度を持たせるか（F4、連続値）
3. **足場の個別化**: 6つの足場機能のどれを、どの強さで、いつ発動するか（F7、随伴的）
4. **認知方略の個別化**: どの方略を、どの条件で、どの段階で適用するか（F6、条件付き）
5. **自己調整支援の個別化**: SRLのどの位相に、どの発達段階で、どう介入するか（F5）
6. **動機づけの個別化**: 3欲求のどれが充足不足か、どう支えるか（F8×F12）
7. **学習プロセスの個別化**: Kolbの4段階をどの順序で経由するか（F3）

**これが「世界最高峰の個別最適化」の実体である。** 一人ひとりの学習者について、この7層すべてが異なる設定値を持ち、リアルタイムに変化する。
